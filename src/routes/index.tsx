...
import { ArrowRight, Cpu, Headphones, Snowflake, Clock, Wifi, Users, Star, Sparkles } from "lucide-react";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Haldwani's most advanced gaming cafe. PS5, PS4, PC & VR — all under one roof at Heera Nagar, Jail Road. Book your slot today." },
    ...
    <div className="relative">
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 py-16 grid lg:grid-cols-2 gap-10 items-center">
          <div className="relative z-10">
            ...
            <p className="mt-6 max-w-lg text-lg text-muted-foreground">
              ...
              <span className="block mt-1 font-accent uppercase tracking-wider text-accent">PS5 • PS4 • PC • VR — All Under One Roof</span>
            </p>
            <div className="mt-8 flex flex-wrap gap-4">
              <a href="https://wa.me/919876543210" target="_blank" rel="noreferrer" className="group inline-flex items-center gap-2 rounded-md bg-gradient-pink px-7 py-4 font-accent font-bold uppercase tracking-wider text-white shadow-neon-pink hover:scale-105 transition">
                Book Your Slot <ArrowRight size={18} className="group-hover:translate-x-1 transition" />
              </a>
              <Link to="/services" className="inline-flex items-center gap-2 rounded-md glass-card px-7 py-4 font-accent font-bold uppercase tracking-wider text-foreground hover:shadow-neon-blue transition">
                Explore Services
              </Link>
            </div>
          </div>
          ...
          {[
            ...
            <div key={s.label} className="text-center">
              <div className="font-display text-3xl sm:text-4xl font-black bg-gradient-neon bg-clip-text text-transparent">
                {typeof s.value === "number" && s.value < 10 ? `${s.value}${s.suffix}` : <><Counter to={s.value as number} suffix={s.suffix} /></>}
              </div>
              <div className="mt-1 font-accent text-xs uppercase tracking-widest text-muted-foreground">{s.label}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 py-24">
        <div className="text-center mb-14">
          ...
          <h2 className="mt-2 font-display text-4xl sm:text-5xl font-black uppercase">Why Anytime Gaming?</h2>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((f) => (
            <div key={f.title} className="group relative glass-card p-7 hover:-translate-y-2 transition-all duration-300 hover:shadow-neon">
      ...
      <section className="relative py-24">
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6">
          <div className="flex items-end justify-between mb-12 flex-wrap gap-4">
            <div>
              <p className="font-accent uppercase tracking-[0.3em] text-sm text-accent">What we offer</p>
              <h2 className="mt-2 font-display text-4xl sm:text-5xl font-black uppercase">Pick Your Platform</h2>
            </div>
            <Link to="/services" className="font-accent uppercase tracking-wider text-sm text-accent hover:text-neon-pink flex items-center gap-2">
              View all <ArrowRight size={16} />
            </Link>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {services.map((s) => (
              <Link key={s.tag} to="/services" className="group glass-card p-6 hover:-translate-y-2 transition-all hover:shadow-neon-pink">
                <div className="font-display text-5xl font-black bg-gradient-pink bg-clip-text text-transparent">{s.tag}</div>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 py-24">
        <div className="text-center mb-12">
          <p className="font-accent uppercase tracking-[0.3em] text-sm text-accent">Inside the arena</p>
        ...
        <div className="mt-8 text-center">
          <Link to="/gallery" className="inline-flex items-center gap-2 glass-card px-6 py-3 font-accent font-bold uppercase tracking-wider text-sm hover:shadow-neon transition">
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 py-24">
        <div className="text-center mb-12">
          <p className="font-accent uppercase tracking-[0.3em] text-sm text-accent">Word on the street</p>
          <h2 className="mt-2 font-display text-4xl sm:text-5xl font-black uppercase">Loved by Gamers</h2>
        </div>
        <div className="grid md:grid-cols-3 gap-5">
          {[
            { name: "Rohan S.", quote: "Best gaming setup in Haldwani, hands down. RTX rigs run BGMI buttery smooth." },
            { name: "Priya M.", quote: "VR was insane! Staff was super friendly and the AC kept us going for hours." },
            { name: "Aman K.", quote: "Birthday party at Anytime was lit. PS5 + snacks + neon vibes = perfect." },
          ].map((r) => (
            <div key={r.name} className="glass-card p-6 hover:shadow-neon transition">
              ...
              <p className="mt-4 font-accent uppercase tracking-wider text-xs text-accent">— {r.name}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="relative mx-auto max-w-7xl px-4 sm:px-6 py-24">
        <div className="relative overflow-hidden rounded-3xl glass-card p-12 md:p-20 text-center">
          ...
          <Particles count={20} />
          <div className="relative">
            <h2 className="font-display text-5xl sm:text-7xl font-black uppercase text-glow">Ready to Play?</h2>
            <p className="mt-4 text-lg text-muted-foreground">Your next gaming session is one click away.</p>
            <a href="https://wa.me/919876543210" target="_blank" rel="noreferrer" className="mt-8 inline-flex items-center gap-3 rounded-md bg-gradient-pink px-8 py-4 font-accent font-bold uppercase tracking-wider text-white shadow-neon-pink hover:scale-105 transition">
      ...
      {popup && (
        <div className="fixed bottom-6 left-6 z-50 max-w-sm animate-fade-in">
          <div className="glass-card p-5 shadow-neon-pink relative">
            <button onClick={() => setPopup(false)} className="absolute top-2 right-2 text-muted-foreground hover:text-foreground text-xs" aria-label="Close">✕</button>
            <p className="font-accent uppercase tracking-widest text-xs text-neon-pink">🎉 New Tournament Alert</p>
            ...
            <div className="mt-3 flex gap-2">
              <Link to="/tournaments" onClick={() => setPopup(false)} className="flex-1 text-center rounded bg-gradient-pink px-3 py-2 text-xs font-accent font-bold uppercase">Register</Link>
              <button onClick={() => setPopup(false)} className="rounded glass-card px-3 py-2 text-xs font-accent uppercase">Later</button>
            </div>
          </div>
        </div>
      )}
