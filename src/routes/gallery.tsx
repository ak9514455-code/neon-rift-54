...
export const Route = createFileRoute("/gallery")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Take a look inside Haldwani's most advanced gaming cafe. Setups, events, and the neon vibe." },
      { property: "og:title", content: "Gallery — Inside the Arena" },
      { property: "og:description", content: "Step inside Anytime Gaming Cafe — neon stations, events, and squad sessions." },
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-16">
      <header className="text-center mb-12">
        ...
        <h1 className="mt-3 font-display text-5xl sm:text-7xl font-black uppercase text-glow">Inside the Arena</h1>
      ...
      <section className="mt-20">
        <h2 className="font-display text-3xl sm:text-4xl font-black uppercase mb-8 text-center">🎬 Watch the Vibe</h2>
        <div className="grid md:grid-cols-2 gap-6">
          {["Cafe Interior", "Gaming Session"].map((label) => (
            <div key={label} className="relative aspect-[9/16] md:aspect-video glass-card overflow-hidden group cursor-pointer">
              ...
              <div className="absolute bottom-4 left-4 font-accent uppercase tracking-wider text-sm">{label}</div>
            </div>
          ))}
        </div>
        <p className="mt-4 text-center text-sm text-muted-foreground">Embed live reels from <a href="https://instagram.com/anytimegaming_hld" className="text-accent">@anytimegaming_hld</a></p>
      </section>
    </div>
  );
}
